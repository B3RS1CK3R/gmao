...
if (!isset($_POST['csrf_token']) || !validate_csrf($_POST['csrf_token'])) {
    header('Location: ?page=contractor_edit&id=' . $id . '&err=' . urlencode(t('csrf_invalid')));
    exit();
}

$company_name = trim($_POST['company_name'] ?? '');
if (empty($company_name)) {
    header('Location: ?page=contractor_edit&id=' . $id . '&err=' . urlencode(t('required_fields_missing')));
    exit();
}

// Vérifier si le nom existe déjà (exclure l'élément en cours)
$check = $pdo->prepare("SELECT id FROM contractors WHERE company_name = ? AND id != ?");
$check->execute([$company_name, $id]);
if ($check->fetch()) {
    header('Location: ?page=contractor_edit&id=' . $id . '&err=' . urlencode(t('contractor_already_exists')));
    exit();
}
...
try {
    ...
    $sql = "UPDATE contractors SET
        ...
        alert_days_before_1 = ?,
        alert_days_before_2 = ?,
        alert_sidebar_intervention = ?,
        alert_popup_intervention = ?,
        alert_email_intervention = ?,
        alert_sidebar_maintenance = ?,
        alert_popup_maintenance = ?,
        alert_email_maintenance = ?
    WHERE id = ?";
    ...
    // Gestion des équipements assignés - Supprimer toutes les associations puis recréer
    $pdo->prepare("DELETE FROM contractor_equipment WHERE contractor_id = ?")->execute([$id]);
    ...
    if (isset($_POST['equipment_ids']) && is_array($_POST['equipment_ids'])) {
        ...
        foreach ($_POST['equipment_ids'] as $equipment_id) {
            ...
            if ($equipment_id > 0) {
                $insertStmt->execute([$id, $equipment_id]);
            }
        }
    }
    ...
    logUserAction($_SESSION['user_id'], 'contractor_updated', "Prestataire modifié: $company_name (ID: $id)");
    header('Location: ?page=contractors&msg=' . urlencode(t('save_success')));

} catch (Exception $e) {
    $pdo->rollBack();
    header('Location: ?page=contractor_edit&id=' . $id . '&err=' . urlencode($e->getMessage()));
}
exit();