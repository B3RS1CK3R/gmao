<?php
// pages/preventive_delete.php - Confirmation d'annulation d'une maintenance préventive
...
// Récupération de la maintenance avec gestion des colonnes
try {
    $stmt = $pdo->prepare("
    ...
    // Si la colonne task_status n'existe pas, faire une requête sans
    $stmt = $pdo->prepare("
        SELECT pm.id, pm.equipment_id, pm.title, pm.frequency_days, pm.next_due, pm.task_number,
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-trash-alt"></i> <?php echo t('delete_preventive'); ?></h2>
    ...
    <?php if ($has_status && ($pm['task_status'] ?? 'pending') == 'completed'): ?>
        <div class="alert alert-warning">
            <i class="fas fa-exclamation-triangle"></i> 
            <?php echo t('cannot_delete_completed_preventive'); ?>
        </div>
    ...
    <?php else: ?>
        <div class="delete-card">
            ...
            <div class="card-body p-4">
                <div class="alert alert-warning">
                    <i class="fas fa-exclamation-triangle"></i>
                    <?php echo t('delete_confirm_preventive'); ?>
                </div>

                <div class="info-row">
                    <span class="label"><?php echo t('task_number'); ?> :</span>
                    <?php echo htmlspecialchars($pm['task_number'] ?? '#' . $pm['id']); ?>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                    ...
                    <?php echo !empty($pm['next_due']) ? format_date_local($pm['next_due'], 'short') : '-'; ?>
                </div>

                <hr>

                <p class="text-muted"><i class="fas fa-info-circle"></i> <?php echo t('delete_warning_preventive'); ?></p>

                <form method="POST" action="?page=preventive_delete_action">
                    <?= csrf_input() ?>
