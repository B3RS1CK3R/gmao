<?php
// pages/preventive_complete.php - Terminer une maintenance préventive
...
// Récupération de la tâche avec équipement
$stmt = $pdo->prepare("
...
$partsStmt = $pdo->prepare("
    ...
    WHERE sm.related_type = 'preventive' AND sm.related_id = ?
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (!isset($_POST['csrf_token']) || !validate_csrf($_POST['csrf_token'])) {
        ...
        if ($stmt->execute([$duration, $report, $id])) {
            logUserAction($_SESSION['user_id'], 'preventive_completed', "Preventive ID: $id marked as completed");
...
<style>
    ...
    .confirm-header { background: linear-gradient(135deg, #28a745, #1e7e34); color: white; padding: 15px 20px; font-weight: bold; }
...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2><i class="fas fa-check-circle"></i> <?php echo t('complete_maintenance'); ?></h2>
        <a href="?page=preventive_view&id=<?php echo $id; ?>" class="btn btn-secondary">
                ...
                <div class="card-body p-4">
                    <form method="POST">
                        ...
                        <div class="info-row">
                            ...
                            <strong><?php echo htmlspecialchars($pm['task_number'] ?? '#' . $pm['id']); ?></strong>
                        </div>
                        <div class="info-row">
                            <span class="label"><?php echo t('equipment'); ?> :</span>
                            <?php echo htmlspecialchars($pm['equipment_name']); ?>
                            <br><span class="text-muted" style="margin-left:150px;"><?php echo htmlspecialchars($pm['equipment_code']); ?></span>
                        </div>
                        <div class="info-row">
