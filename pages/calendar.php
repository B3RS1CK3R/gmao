...
<head>
    ...
    <title>GMAO Pro - <?php echo t('calendar_view'); ?></title>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDrop: function(info) { updateEventDate(info.event.id, info.event.startStr); },
        eventResize: function(info) { updateEventDate(info.event.id, info.event.startStr); },
        height: 'auto',
...
function changeView(view) { if(calendar) calendar.changeView(view); }
...
function showEventDetails(event) {
    ...
    switch(priority) {
        ...
        case 'medium': headerClass = 'modal-header-medium'; priorityBadge = '<span class="badge bg-info"><?php echo t('medium'); ?></span>'; break;
        default: headerClass = 'modal-header-low'; priorityBadge = '<span class="badge bg-secondary"><?php echo t('low'); ?></span>';
    }
    ...
    if(props.zone || props.localisation) {
        locationHtml = `<div class="row mb-2"><div class="col-4"><strong><?php echo t('location'); ?> :</strong></div><div class="col-8">${props.zone ? props.zone + (props.localisation ? ' / ' : '') : ''}${props.localisation || '-'}</div></div>`;
    }
    ...
    document.getElementById('modalBody').innerHTML = `
        ...
        <div class="row mb-2"><div class="col-4"><strong><?php echo t('equipment'); ?> :</strong></div><div class="col-8">${props.equipment_name}<br><small class="text-muted"><?php echo t('code'); ?>: ${props.equipment_code}</small></div></div>
        ${locationHtml}
        <div class="row mb-2"><div class="col-4"><strong><?php echo t('technician'); ?> :</strong></div><div class="col-8">${props.technician_name}<br>${props.technician_specialty ? '<small class="text-muted">' + props.technician_specialty + '</small>' : ''}</div></div>
        ${props.planned_duration ? `<div class="row mb-2"><div class="col-4"><strong><?php echo t('planned_duration'); ?> :</strong></div><div class="col-8">${props.planned_duration}</div></div>` : ''}
        ...
        ${props.completion_report ? `<div class="row mb-2"><div class="col-4"><strong><?php echo t('report'); ?> :</strong></div><div class="col-8"><small class="text-success"><?php echo t('report_available'); ?></small></div></div>` : ''}
    `;
...
function updateEventDate(eventId, newDate) {
    ...
    .then(data => { if(!data.success) { alert('<?php echo t('error_update_date'); ?>'); location.reload(); } })
...
document.getElementById('technicianFilter').addEventListener('change', function() {
    ...
    if(calendar) {
        if(technicianId) {
            const filteredEvents = currentEvents.filter(event => event.extendedProps.technician_id == technicianId);
